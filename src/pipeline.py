...
def run(model, dataset, epochs=5, sub_epochs=30, is_active_learning=False, heuristic=None, n_batches=20):
    param_path = "params/"
    ...
    best_metrics = run_learning(net, device, optimizer, scheduler, 
                      ...
                      epochs=epochs, sub_epochs=sub_epochs, active_learning=is_active_learning, heuristic=heuristic, labeled_idx_df=labeled_idx_df, n_batches=n_batches)
...
if __name__ == "__main__":
    ...
    heuristics = ["largest_margin", "smallest_margin", "least_confidence", "mc_dropout", "representative_sampling", "representative_mc_dropout",]
    ...
    epochs = 20
    active = False
    heuristic = heuristics[-1] # if active
    ...
    if active:
        ...
        best_metrics = run(model=model, dataset=dataset, epochs=experiments, sub_epochs=epochs, is_active_learning=active, heuristic=heuristic, n_batches=n_batches) 
        ...
        best_metrics = run(model=model, dataset=dataset, epochs=experiments, sub_epochs=epochs, is_active_learning=active) 
